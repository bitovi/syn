...
define('syn/drag', [
    ...
    var mouseMove = function (point, win, last) {
            ...
            if (syn.support.pointerEvents && el != null) { syn.trigger(el, 'pointermove', point); }
            if (syn.support.touchEvents && el != null) { syn.trigger(el, 'touchmove', point); }
			if(el != null){ syn.trigger(el, 'mousemove', point); }
			
            return el;
        }, createEventAtPoint = function (event, point, win) {
            var el = elementFromPoint(point, win);
			
			if(el == null){ el = win.document.querySelector('body'); }
			
            syn.trigger(el, event, point);
            return el;
