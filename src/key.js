	...
	key: function (element, options) {
		...
		return new Promise((resolve) => {
			...
			var activeElement = h.getWindow(element)
				...
				key = syn.key.convert[options] || options,
				...
				keypressOptions = getKeyOptions(key, 'keypress');

			if (runDefaults) {
				//if the browser doesn't create keypresses for this key, run default
				if (!keypressOptions) {
					defaultResult = getKeyDefault(key)
...
syn.helpers.extend(syn.events.kinds, {
	key: {
		...
		// creates a key event
		create: function (type, options, element) { //Everyone Else
...
syn.helpers.extend(syn.key, {
	...
	defaults: {
		...
		'a': function (options, scope, key, force, sel) {
			if (syn.keysBeingHeldDown.ctrlKey) {
				selectText(this, 0, getText(this)
					.length);
			} else {
				syn.key.defaults.character.apply(this, arguments);
			}
		...
		'right': function (options, scope, key, force, sel) {
			if (syn.typeable.test(this)) {
				if (syn.keysBeingHeldDown.shiftKey) {
					selectText(this, sel.start, sel.end + 1 > getText(this)
						.length ? getText(this)
						.length : sel.end + 1);
				} else {
					selectText(this, sel.end + 1 > getText(this)
						.length ? getText(this)
						.length : sel.end + 1);
				}
			}
	...
	// these are helpers that are used by tests
	helpers: {
		//returns the special key if special (this is used by the recorder)
