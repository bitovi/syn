//@steal-clean //avoid overly complex expressions
//Safari doesn't send mouse/click on option
/*jslint evil: true */
/*global REPLAY: true, ADD: true, Recorder: true, commands: false, justKey: true */
$(function () {
	Syn.autoDelay = true;
	REPLAY = false;
	ADD = true;
	...
	var commands = [],
		...
		h = {
			commandsText: function (func) {
				...
				for (var i = 0; i < commands.length; i++) {
					...
					text.push(func ? "" : "<div>", i > 0 ? "   ." : "Syn.", command.type, "(", args.join(", "), ")\n", func ? "" : "</div>");
					prev = command;
				}
				return text.join("");
			},
			addCode: function (type, options, target) {
				...
				if (last.type == type && type == 'type' && last.selector == selector) {
					...
					if (last.type == 'type' && type == 'click' && last.options.lastIndexOf("\\r") == last.options.length - 3 && last.selector == selector) {} else {
						commands.push({
							type: type,
							options: options,
							selector: selector
		...
		mouseupH = function (ev) {
			if (/option/i.test(ev.target.nodeName)) {
				...
				if (!mousemove || (lastX == ev.pageX && lastY == ev.pageY)) {
					h.addCode("click", undefined, ev.target);
				} else {
					if (mousemove > 2 && mousedown) {
	...
	$("<iframe src='demo.html'></iframe>").load(function () {
		...
		keytarget = null;
		$(frameWindow.document).keydown(h.keydown).keyup(h.keyup).mousedown(mousedownH).mousemove(mousemoveH).mouseup(mouseupH).change(changeH).click(function (ev) {
